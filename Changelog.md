## [Alpha V1.378] - 2026-05-07 00:45:00

### 🎨 Style: 인사이트 페이지 포트폴리오 구성 블록 최상단 이동 및 아이콘 정리
- **Summary**: 인사이트 페이지 레이아웃 개선 및 이전 아이콘 작업 잔여 파일 정리
- **Detail** :
  - **포트폴리오 구성 블록 최상단 배치**: `/insights` 페이지에서 `PortfolioCompositionBlock`을 AI 분석 블록 위 최상단으로 이동하여, 포트폴리오 현황을 가장 먼저 확인할 수 있도록 개선
  - **아이콘 변환 스크립트 추가**: `scripts/convert-icon.js` 아이콘 생성 유틸리티 추가
  - **불필요 파비콘 파일 제거**: `src/app/--favicon.ico` 삭제
  - **패키지 의존성 업데이트**: sharp 등 이미지 처리 관련 의존성 갱신
- **Build Time**: 2026-05-07 00:45:00

## [Alpha V1.377] - 2026-04-28 01:44:45

### 🚀 Update: 투자 리포트 UI/UX 개선 및 전체 성과 지표 통합
- **Summary**: 메일 및 웹 대시보드 리포트에 전체 기간 포트폴리오 현황 추가 및 시각적 차별화 적용
- **Detail** :
  - **전체 기간 누적 성과 추가**: 기존 월간/주간 리포트에 더해, 최초 투자 시점부터 현재까지의 총 투자금, 총 평가금, 전체 수익률 및 평가손익을 보여주는 `overallSummary` 데이터 파이프라인 신설
  - **시각적 차별화 (Gold vs Indigo)**: 전체 포트폴리오 현황은 골드 프리미엄 스타일(그라디언트 보더 및 골드 글로우 효과)로, 특정 기간 성과는 인디고 블루 컨테이너 스타일로 분리하여 유저가 지표를 명확히 구분할 수 있도록 리디자인
  - **기간 한정 리포트 안내 배너**: 해당 리포트가 전체 성과가 아닌 특정 기간에 한정된 성과임을 알리는 앰버(Amber) 색상의 경고 배너 추가
  - **매수가(AvgPrice) 데이터 매핑 버그 수정**: DB 스냅샷 구조(`buy_price`)와 리포트 로직 간의 키 불일치로 인해 매수가가 0으로 나오던 문제를 해결 (`buy_price || avg_price || avgPrice` 폴백 적용)
...
- **Summary**: 리포트 대시보드에서 해외 주식(US) 종목의 과거 주가 데이터를 불러오지 못하던 근본 원인 3가지 해결
- **Detail** :
  - **API 응답 필드 불일치 수정**: `HHDFS76240000` API의 응답 필드(`xymd`/`clos`)를 기존 파싱 코드(`stck_bsop_date`/`ovrs_nmix_prpr`)와 호환되도록 정규화 변환 레이어 추가
  - **거래소 코드 자동 Fallback**: 1차 거래소(NAS/NYS/AMS) 조회 실패 시 다른 거래소로 자동 재시도하는 3단계 fallback 로직 구현
  - **거래소 심볼 매핑 대폭 확장**: NYSE 14개→60개+, AMEX 24개→40개+ (사용자 포트폴리오 종목 포함)
  - **투자금/평가금 차트 이중 환율 버그 수정**: 프론트에서 서버가 이미 원화 변환한 데이터를 이중 변환하던 코드 제거
  - **시세 조회 실패 시 fallback 개선**: 평가금을 0원이 아닌 매입단가로 대체하여 차트 왜곡 방지
  - **DB 캐시 무효화**: noCache 파라미터 추가로 오염된 캐시 데이터 강제 재계산
- **Build Time**: 2026-04-26 23:40:56
